...
// Populate the GtkTreeStore with directory/file nodes from arz records.
// Records are sorted alphabetically by path and inserted into a tree
// structure with directories as intermediate nodes and .dbr files as leaves.
//
// st: dialog state containing the tree store and arz file
static void
...
// Called when the tree selection changes.  If a leaf node (.dbr record) is
// selected, reads its variables and populates the variable list store.
//
// sel:  the tree selection
// data: DatabaseDialogState pointer
static void
on_tree_selection_changed(GtkTreeSelection *sel, gpointer data)
{
  DatabaseDialogState *st = data;
...
// Tree model filter visibility function.  Returns TRUE if the node (or any
// descendant) matches the current search text.
//
// model: the underlying tree model
// iter:  the row to test
// data:  DatabaseDialogState pointer
...
// and triggers a filter refilter.
//
