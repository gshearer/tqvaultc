      ...
      for(int i = 0; i < num_items; i++)
      {
        ...
        if(vault->sacks[s].items[i].base_name)
        {
          ...
          else
          {
            ...
            if(iw_found && iw > 0)
              vault->sacks[s].items[i].width = iw;

            if(ih_found && ih > 0)
              vault->sacks[s].items[i].height = ih;
          }
        }
      }
    }
...
int
vault_save_json(TQVault *vault, const char *filepath)
{
  if(!vault || !filepath)
    return(-1);
...
// x: grid column
// y: grid row
// returns: pointer to the item occupying that cell, or NULL if empty
TQVaultItem *
vault_get_item_at(TQVault *vault, int sack, int x, int y)
{
  if(!vault || sack < 0 || sack >= vault->num_sacks)
    return(NULL);
...
// vault_item_free_strings - free all dynamically allocated string fields in a vault item
// it: vault item whose strings to free
void
vault_item_free_strings(TQVaultItem *it)
{
...
// vault: vault to free (no-op if NULL)
void
vault_free(TQVault *vault)
{
  if(!vault)
    return;
