...
// haystack_len: length of the haystack buffer.
// needle: the null-terminated string to search for.
// Returns a pointer to the first match, or NULL if not found.
static const char *
ci_strstr(const char *haystack, size_t haystack_len, const char *needle)
{
...
static int
ends_with_ci(const char *str, const char *suffix)
{
...
// Process a single .arc file: list all entries, search .txt files for
// "x4tag", and report matches with context lines.
// path: the file path to the .arc archive.
static void
process_arc(const char *path)
{
  printf("========================================\n");
  printf("Loading: %s\n", path);
  printf("========================================\n");
...
// Entry point. Processes one or more .arc files, searching each for
// XPack4 translation tags.
// argc: argument count.
// argv: argument vector; argv[1..] are .arc file paths.
// Returns 0 on success, 1 on error (no arguments).
int
