...
// Entry point. Loads a .chr file and prints character name and all 12
// equipment slots with prefix, suffix, relic, and relic bonus info.
// argc: argument count.
// argv: argument vector; argv[1] is the .chr file path.
...
main(int argc, char **argv)
{
  ...
  for(int i = 0; i < 12; i++)
  {
    if(character->equipment[i])
    {
      ...
      if(character->equipment[i]->prefix_name)
        printf("  Prefix: %s\n", character->equipment[i]->prefix_name);

      if(character->equipment[i]->suffix_name)
        printf("  Suffix: %s\n", character->equipment[i]->suffix_name);

      if(character->equipment[i]->relic_name)
        printf("  Relic: %s\n", character->equipment[i]->relic_name);

      if(character->equipment[i]->relic_bonus)
        printf("  Relic Bonus: %s\n", character->equipment[i]->relic_bonus);

      if(character->equipment[i]->relic_name2)
        printf("  Relic2: %s\n", character->equipment[i]->relic_name2);

      if(character->equipment[i]->relic_bonus2)
        printf("  Relic2 Bonus: %s\n", character->equipment[i]->relic_bonus2);
    }
    else
    {
      printf("Slot %d: Empty\n", i);
