...
// Frees all heap-allocated string fields in a vault item.
// item: pointer to the item whose strings should be freed (may be NULL).
void
...
// Entry point. Loads a .chr file, round-trips it through save, and
// compares structural keys in the inventory section.
// argc: argument count.
// argv: argument vector; argv[1] is optional .chr file path.
  ...
  if(ret == 0)
  {
    ...
    while(off + 4 <= (size_t)sz)
    {
      ...
      if(klen > 0 && klen < 256 && off + 4 + klen <= (size_t)sz)
      {
        ...
        {
          if(data[off+4+i] < 32 || data[off+4+i] > 126)
          {
            printable = 0;
            break;
          }
