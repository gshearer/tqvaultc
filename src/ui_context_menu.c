...
//
// widgets: application widget state containing container references
static void
mark_context_dirty(AppWidgets *widgets)
{
...
//
// path: DBR path to check
// returns: "Charm" or "Relic"
static const char *
relic_type_label_ui(const char *path)
{
  if(strcasestr(path, "charm"))
    return("Charm");

  if(strcasestr(path, "animalrelic"))
    return("Charm");
...
//
// item_path: DBR path to the item
// returns: internal pointer (do not free), or NULL
static const char *
get_bonus_table_path(const char *item_path)
{
...
//
// table_path: DBR path to the bonus randomizer table
// current_bonus: currently assigned bonus path (for marking), or NULL
// action_name: GAction name prefix for menu items
// tr: translation context for resolving display names
// returns: a new GMenu (caller owns), or NULL if the table has no entries
static GMenu *
build_bonus_submenu(const char *table_path, const char *current_bonus,
...
// drawing_area: the widget that was right-clicked
// item: vault item under cursor, or NULL
// equip_item: equipment item under cursor, or NULL
// source: container type the item belongs to
// sack_idx: sack index for vault items
// equip_slot: equipment slot index, or -1
// x, y: click coordinates for popover positioning
void
...
// Copy the DBR path of the right-clicked item to the clipboard.
//
// action: the GSimpleAction (unused)
// param: action parameter (unused)
// data: AppWidgets pointer
static void
on_copy_dbr_path(GSimpleAction *action, GVariant *param, gpointer data)
{
...
// Copy the right-clicked item as JSON to the clipboard.
//
// action: the GSimpleAction (unused)
// param: action parameter (unused)
// data: AppWidgets pointer
static void
on_copy_item_json(GSimpleAction *action, GVariant *param, gpointer data)
{
  (void)action; (void)param;
  AppWidgets *widgets = data;
...
// Copy the right-clicked item to the cursor (pick up a copy).
//
// action: the GSimpleAction (unused)
// param: action parameter (unused)
// data: AppWidgets pointer
static void
on_item_copy(GSimpleAction *action, GVariant *param, gpointer data)
{
  (void)action; (void)param;
  AppWidgets *widgets = data;
...
// Duplicate the right-clicked item onto the cursor.
//
// action: the GSimpleAction (unused)
// param: action parameter (unused)
// data: AppWidgets pointer
static void
on_item_duplicate(GSimpleAction *action, GVariant *param, gpointer data)
{
  (void)action; (void)param;
  AppWidgets *widgets = data;
...
// Delete the right-clicked item from its container.
//
// action: the GSimpleAction (unused)
// param: action parameter (unused)
// data: AppWidgets pointer
static void
on_item_delete(GSimpleAction *action, GVariant *param, gpointer data)
{
  (void)action; (void)param;
  AppWidgets *widgets = data;
  ...
  // Clear compare if the deleted item was the compare item
  if(widgets->compare_active)
  {
    if(widgets->context_equip_slot >= 0 &&
       widgets->compare_source == CONTAINER_EQUIP &&
       widgets->compare_equip_slot == widgets->context_equip_slot)
      clear_compare_item(widgets);
    else if(widgets->context_item &&
            widgets->compare_source == widgets->context_source &&
            widgets->compare_sack_idx == widgets->context_sack_idx &&
            widgets->context_item->point_x == widgets->compare_item.point_x &&
            widgets->context_item->point_y == widgets->compare_item.point_y)
      clear_compare_item(widgets);
  }
...
// data: AppWidgets pointer
static void
on_item_remove_relic(GSimpleAction *action, GVariant *param, gpointer data)
{
  (void)action; (void)param;
  AppWidgets *widgets = data;
  ...
  else if(widgets->context_item)
  {
    ...
    if(!it->relic_name || !it->relic_name[0])
      return;
...
// data: AppWidgets pointer
static void
on_item_remove_relic2(GSimpleAction *action, GVariant *param, gpointer data)
{
  (void)action; (void)param;
  AppWidgets *widgets = data;
  ...
  if(widgets->context_equip_slot >= 0 && widgets->context_equip_item)
  {
    ...
    if(!eq->relic_name2 || !eq->relic_name2[0])
      return;
  ...
  else if(widgets->context_item)
  {
    ...
    if(!it->relic_name2 || !it->relic_name2[0])
      return;
...
//
// action: the GSimpleAction (unused)
// param: GVariant string containing the affix path
// data: AppWidgets pointer
static void
on_set_prefix(GSimpleAction *action, GVariant *param, gpointer data)
{
  (void)action;
  AppWidgets *widgets = data;
  const char *affix_path = g_variant_get_string(param, NULL);

  if(!affix_path || !affix_path[0])
    return;
...
// data: AppWidgets pointer
static void
on_remove_prefix(GSimpleAction *action, GVariant *param, gpointer data)
{
  (void)action; (void)param;
  AppWidgets *widgets = data;
...
// data: AppWidgets pointer
static void
on_set_suffix(GSimpleAction *action, GVariant *param, gpointer data)
{
  (void)action;
  AppWidgets *widgets = data;
  const char *affix_path = g_variant_get_string(param, NULL);
...
// data: AppWidgets pointer
static void
on_remove_suffix(GSimpleAction *action, GVariant *param, gpointer data)
{
  (void)action; (void)param;
  AppWidgets *widgets = data;
...
//
// action: the GSimpleAction (unused)
// param: GVariant string containing the bonus path
// data: AppWidgets pointer
static void
on_set_relic_bonus(GSimpleAction *action, GVariant *param, gpointer data)
{
  (void)action;
  AppWidgets *widgets = data;
...
// data: AppWidgets pointer
static void
on_set_relic_bonus2(GSimpleAction *action, GVariant *param, gpointer data)
{
  (void)action;
  AppWidgets *widgets = data;
  const char *bonus_path = g_variant_get_string(param, NULL);
...
// Launch the affix modification dialog for the right-clicked item.
//
// action: the GSimpleAction (unused)
// param: action parameter (unused)
// data: AppWidgets pointer
static void
on_modify_affixes(GSimpleAction *action, GVariant *param, gpointer data)
{
  (void)action; (void)param;
  AppWidgets *widgets = data;
...
// Launch the forge affix dialog for the right-clicked item.
//
// action: the GSimpleAction (unused)
// param: action parameter (unused)
// data: AppWidgets pointer
static void
on_forge_affixes(GSimpleAction *action, GVariant *param, gpointer data)
{
  (void)action; (void)param;
  AppWidgets *widgets = data;
  const char *base = NULL;
...
// Callback for quantity slider value changes; updates the live label.
//
// range: the GtkRange slider widget
// user_data: the GtkWidget label to update
static void
on_qty_scale_changed(GtkRange *range, gpointer user_data)
{
...
// Apply the selected quantity and close the dialog.
//
// btn: the OK button (unused)
// user_data: the dialog window widget
static void
on_set_qty_ok(GtkButton *btn, gpointer user_data)
{
  (void)btn;
...
// Show the "Set Quantity" dialog for stackable items / relic shards.
//
// action: the GSimpleAction (unused)
// param: action parameter (unused)
// user_data: AppWidgets pointer
static void
on_set_stack_quantity(GSimpleAction *action, GVariant *param, gpointer user_data)
{
  (void)action; (void)param;
  ...
  int max_val   = is_relic ? relic_max_shards(it->base_name) : 100;
...
// Register all context menu GActions with the application.
//
// app: the GtkApplication to register actions on
...
{
  static const struct { const char *name; GCallback cb; const GVariantType *type; } acts[] = {
