...
// cap: initial buffer capacity in bytes
static void
...
// Write a uint32_t value to the ByteBuf.
// b: pointer to ByteBuf
...
// Write a length-prefixed string to the ByteBuf.
// b: pointer to ByteBuf
// s: null-terminated string to write (may be NULL for zero-length)
static void
...
// Read a uint32_t from data at the given byte offset.
// data: raw byte buffer
// off: byte offset to read from
// Returns the 32-bit value at that offset.
static uint32_t
read_u32(const uint8_t *data, size_t off)
{
  uint32_t val;
...
// Read a length-prefixed string; advances *off.
// data: raw byte buffer
// len: total buffer length
// off: pointer to current offset (advanced past the string on return)
// Returns malloc'd string or NULL on error.
static char *
read_str(const uint8_t *data, size_t len, size_t *off)
{
  if(*off + 4 > len)
    return(NULL);
...
// data: raw byte buffer
// len: total buffer length
// off: pointer to current offset (advanced on return)
// expected: the key string to match against
// Returns true if the key matches, false otherwise.
static bool
expect_key(const uint8_t *data, size_t len, size_t *off, const char *expected)
{
...
// Initialize a QuestTokenSet to empty state.
// set: pointer to QuestTokenSet to initialize
void
...
// Free all tokens and the token array in a QuestTokenSet.
// set: pointer to QuestTokenSet to free
void
...
// Check whether the token set contains the given token string.
// set: pointer to QuestTokenSet to search
// token: token string to look for
...
// Add a token to the set if not already present. Marks set as dirty.
// set: pointer to QuestTokenSet
// token: token string to add (will be strdup'd)
void
...
// Remove a token from the set by swapping with the last element. Marks set as dirty.
// set: pointer to QuestTokenSet
...
// Load quest tokens from a QuestToken.myw binary file.
// filepath: path to the .myw file
// out: pointer to QuestTokenSet to populate (will be initialized)
...
quest_tokens_load(const char *filepath, QuestTokenSet *out)
{
  ...
  // versionNumber + value
  if(!expect_key(data, len, &off, "versionNumber"))
  {
    free(data);
    return(-1);
  }
  ...
  // begin_block + marker
  if(!expect_key(data, len, &off, "begin_block"))
  {
    free(data);
    return(-1);
  }
  ...
  if(!expect_key(data, len, &off, "numberOfTriggerTokens"))
  {
    free(data);
    return(-1);
  }
  ...
  for(uint32_t i = 0; i < count; i++)
  {
    ...
    // name + token string
    if(!expect_key(data, len, &off, "name"))
      break;
    ...
    if(!expect_key(data, len, &off, "fileReferenceCount"))
      break;
    ...
    // end_block + marker
    if(!expect_key(data, len, &off, "end_block"))
      break;
...
// Save quest tokens to a QuestToken.myw binary file.
// filepath: path to the .myw file (existing file is backed up first)
// set: pointer to QuestTokenSet to write
// Returns 0 on success, -1 on error.
int
...
// Build the path to a QuestToken.myw file for a character at a given difficulty.
// char_filepath: path to the character's Player.chr file
// diff: difficulty level (Normal/Epic/Legendary)
// Returns malloc'd path string, or NULL on error. Caller must free.
char *
quest_token_path(const char *char_filepath, QuestDifficulty diff)
{
  if(!char_filepath || diff < 0 || diff >= NUM_DIFFICULTIES)
    return(NULL);
...
// Build the path to the quest state directory for a character at a given difficulty.
// char_filepath: path to the character's Player.chr file
// diff: difficulty level (Normal/Epic/Legendary)
// Returns malloc'd path string, or NULL on error. Caller must free.
char *
...
// Backup a file by copying it to <filepath>.bak.
// filepath: path to the file to back up
// Returns 0 on success (or if file doesn't exist), -1 on error.
int
...
// Zero all hasFired/isPendingFire flags in all .que files in the given directory.
  ...
  while((ent_name = g_dir_read_name(d)) != NULL)
  {
    ...
    for(int t = 0; t < 2; t++)
    {
      ...
      for(size_t off = 0; off + 4 + klen + 4 <= (size_t)fsize; off++)
      {
        ...
        if(slen != (uint32_t)klen)
          continue;

        if(memcmp(data + off + 4, key, klen) != 0)
          continue;
...
// Return the human-readable name for a quest act.
// act: QuestAct enum value
// Returns a static string, or "Unknown" for invalid values.
const char *
quest_act_name(QuestAct act)
{
  ...
  if(act >= 0 && act < NUM_ACTS)
    return(names[act]);

  return("Unknown");
}

// Return the human-readable name for a quest difficulty.
// diff: QuestDifficulty enum value
// Returns a static string, or "Unknown" for invalid values.
const char *
quest_difficulty_name(QuestDifficulty diff)
{
  if(diff >= 0 && diff < NUM_DIFFICULTIES)
    return(diff_dirs[diff]);
...
static const char *const q_rag_sq14[] = {
  "SQ14Completed", NULL
};
static const char *const q_rag_sq15[] = {
  "SQ15Completed", NULL
};
static const char *const q_rag_sq16[] = {
  "SQ16Completed", NULL
};
static const char *const q_rag_sq17[] = {
  "SQ17Completed", "SQ17Completed_Alive", "SQ17ExclamationDone",
  "SQ17GotTeeth", "SQ17QuestOngoing", NULL
};
static const char *const q_rag_sq19[] = {
  "SQ19Completed", "SQ19Completed_Armor", "SQ19ExclamationDone", "SQ19GotQuest", NULL
};
static const char *const q_rag_sq20[] = {
  "SQ20Completed", "SQ20ExclamationDone", "SQ20GotNecklace", NULL
};
static const char *const q_rag_sq21[] = {
  "SQ21Completed", "SQ21ReturnedTabletAB", NULL
};
static const char *const q_rag_sq22[] = {
  "SQ22GotQuest", "SQ22_Completed", "SQ22_DurinUsed", NULL
};

// --- ACT VI: Atlantis ------------------------------------------------------------

static const char *const q_atl_main[] = {
...
static const char *const q_no_tokens[] = { NULL };

// Orient: Olympus main quest has one orphaned token
static const char *const q_orient_olympus[] = {
  "Olympus - Typhon Defeated", NULL
};

// --- ACT VII: Eternal Embers -----------------------------------------------------

static const char *const q_ee_jc001[] = {
...
static const char *const q_ee_jc002[] = {
...
static const char *const q_ee_jc003[] = {
...
static const char *const q_ee_je001[] = {
...
// EE side quests
static const char *const q_ee_sq101[] = {
...
static const char *const q_ee_sq102[] = {
...
static const char *const q_ee_sq104[] = {
...
static const char *const q_ee_sq105[] = {
...
static const char *const q_ee_sq201[] = {
...
static const char *const q_ee_sq202[] = {
...
static const char *const q_ee_sq203[] = {
...
static const char *const q_ee_sq204[] = {
...
static const char *const q_ee_sq205[] = {
...
static const char *const q_ee_sq302[] = {
...
static const char *const q_ee_sq303[] = {
...
static const char *const q_ee_sq401[] = {
...
// Return the quest definitions table and its count.
// count_out: if non-NULL, receives the number of quest definitions
// Returns pointer to the static quest definitions array.
const QuestDef *
quest_get_defs(int *count_out)
{
  if(count_out)
    *count_out = quest_defs_count;
...
// Return the checklist extra definitions table and its count.
// count_out: if non-NULL, receives the number of checklist extra definitions
// Returns pointer to the static checklist extras array.
const ChecklistExtraDef *
checklist_get_extras(int *count_out)
{
  if(count_out)
    *count_out = checklist_extra_count;

  return(checklist_extra_defs);
}

// Return the human-readable name for a checklist category.
// cat: ChecklistCategory enum value
// Returns a static string, or "Unknown" for invalid values.
const char *
checklist_category_name(ChecklistCategory cat)
{
  ...
  if(cat >= 0 && cat < NUM_CHECK_CATEGORIES)
    return(names[cat]);
