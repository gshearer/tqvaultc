...
// Free the dialog state allocation.
// data: pointer to StatsDialogState (passed as gpointer)
static void
stats_state_free(gpointer data)
{
  g_free(data);
}
...
// Compute the number of unspent attribute points.
// st: dialog state
// Returns: number of available points
static int
compute_avail(StatsDialogState *st)
{
  int spent = 0;
...
// Update all value labels, available-points label, and button sensitivity.
// st: dialog state
static void
refresh_display(StatsDialogState *st)
{
...
// Callback for the "+" button: increment the attribute's spent points.
// btn: the clicked button (carries "attr-idx" data)
// user_data: StatsDialogState pointer
static void
on_plus_clicked(GtkButton *btn, gpointer user_data)
{
  (void)btn;
...
// Callback for the "-" button: decrement the attribute's spent points.
// btn: the clicked button (carries "attr-idx" data)
// user_data: StatsDialogState pointer
static void
on_minus_clicked(GtkButton *btn, gpointer user_data)
{
  (void)btn;
  StatsDialogState *st = user_data;
  int idx = GPOINTER_TO_INT(g_object_get_data(G_OBJECT(btn), "attr-idx"));
...
// Callback for "Apply" button: write new stat values and close dialog.
...
// Callback for "Cancel" button: close dialog without saving.
// btn: the clicked button (unused)
// user_data: StatsDialogState pointer
static void
...
// Create a horizontal row widget for one attribute stat.
// label_text: display name (e.g. "Strength:")
// idx: attribute index (0-4)
// st: dialog state
// Returns: the row widget
static GtkWidget *
make_attr_row(const char *label_text, int idx, StatsDialogState *st)
{
...
// Show the attributes dialog for the current character.
