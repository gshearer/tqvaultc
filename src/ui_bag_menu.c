...
// ct:      which container type was modified
static void
mark_bag_dirty(AppWidgets *widgets, ContainerType ct)
{
...
// Resolve (container_type, idx) to a sack pointer, cols, and rows.
// Returns NULL if the container/index is invalid or not loaded.
//
// widgets:  application state
// ct:       container type (vault, inventory, bag)
// idx:      sack index within the container
// out_cols: receives the column count
// out_rows: receives the row count
// returns:  pointer to the sack, or NULL on failure
static TQVaultSack *
resolve_sack(AppWidgets *widgets, ContainerType ct, int idx,
             int *out_cols, int *out_rows)
{
  if(ct == CONTAINER_VAULT)
  {
    ...
    if(idx < 0 || idx >= widgets->current_vault->num_sacks)
      return(NULL);
  ...
  if(ct == CONTAINER_INV)
  {
    ...
    if(widgets->current_character->num_inv_sacks < 1)
      return(NULL);
  ...
  if(ct == CONTAINER_BAG)
  {
    ...
    if(si >= widgets->current_character->num_inv_sacks)
      return(NULL);
...
// Parse a destination string like "v:3", "i:0", "b:2" into container
// type + index.
//
// s:   the destination string
// ct:  receives the parsed container type
// idx: receives the parsed index
// returns: true on success, false on parse failure
static bool
parse_dest(const char *s, ContainerType *ct, int *idx)
{
  if(!s || strlen(s) < 3 || s[1] != ':')
    return(false);
...
//
// grid:   occupancy grid (cols * rows booleans)
// cols:   grid width
// rows:   grid height
// item_w: item width in cells
// item_h: item height in cells
// out_x:  receives the found x position
// out_y:  receives the found y position
// returns: true if a free spot was found
static bool
find_free_spot(const bool *grid, int cols, int rows,
...
// grid: occupancy grid
// cols: grid width
// x, y: top-left cell of the item
// w, h: item dimensions in cells
static void
mark_grid(bool *grid, int cols, int x, int y, int w, int h)
{
...
// Build a GMenu submenu listing all valid destination bags for a transfer
// operation, excluding the source bag itself.
//
// widgets:       application state
// action_prefix: GAction name prefix (e.g. "bag-move-to")
// src_ct:        source container type
// src_idx:       source sack index
// returns:       new GMenu (caller owns the reference)
static GMenu *
build_dest_submenu(AppWidgets *widgets, const char *action_prefix,
                   ContainerType src_ct, int src_idx)
{
  ...
  if(widgets->current_vault)
  {
    ...
    for(int i = 0; i < n; i++)
    {
      if(src_ct == CONTAINER_VAULT && i == src_idx)
        continue;
  ...
  if(widgets->current_character)
  {
    ...
    {
      if(src_ct == CONTAINER_BAG && i == src_idx)
        continue;
...
// Transfer (move or copy) all items from the source bag to a destination bag.
// Items are placed at their original position if possible, otherwise at
// the first free spot.  Moved items are removed from the source.
//
// widgets:  application state
// dest_str: destination string (e.g. "v:3")
// is_move:  true to move (remove from source), false to copy
static void
do_transfer(AppWidgets *widgets, const char *dest_str, bool is_move)
{
  ...
  if(!parse_dest(dest_str, &dst_ct, &dst_idx))
    return;
...
// GAction callback: move all items from source bag to the specified dest.
//
// action:    the triggered action (unused)
// parameter: GVariant string with destination (e.g. "v:3")
// user_data: AppWidgets pointer
static void
on_bag_move_to(GSimpleAction *action, GVariant *parameter, gpointer user_data)
{
  (void)action;
...
// GAction callback: copy all items from source bag to the specified dest.
//
// action:    the triggered action (unused)
// parameter: GVariant string with destination (e.g. "v:3")
// user_data: AppWidgets pointer
static void
on_bag_copy_to(GSimpleAction *action, GVariant *parameter, gpointer user_data)
{
  (void)action;
  AppWidgets *widgets = user_data;
  const char *dest = g_variant_get_string(parameter, NULL);
...
// GAction callback: empty (delete all items from) the source bag.
//
// action:    the triggered action (unused)
// parameter: unused
// user_data: AppWidgets pointer
static void
on_bag_empty(GSimpleAction *action, GVariant *parameter, gpointer user_data)
{
...
// Comparison function for auto-arrange: sort by area descending, then
// height, then width.
//
// a, b: pointers to int[4] arrays {index, width, height, area}
// returns: negative/zero/positive for ordering
static int
cmp_items_by_area(const void *a, const void *b)
{
  const int *ia = a, *ib = b;

  // ia[0] = index, ia[1] = width, ia[2] = height, ia[3] = area
  if(ib[3] != ia[3])
    return(ib[3] - ia[3]);  // area descending

  if(ib[2] != ia[2])
    return(ib[2] - ia[2]);  // height descending

  return(ib[1] - ia[1]);    // width descending
}

// GAction callback: auto-arrange items in the source bag by packing
// largest-area items first into a fresh grid.
//
// action:    the triggered action (unused)
// parameter: unused
// user_data: AppWidgets pointer
static void
on_bag_auto_arrange(GSimpleAction *action, GVariant *parameter, gpointer user_data)
{
  (void)action; (void)parameter;
  AppWidgets *widgets = user_data;
...
// GAction callback: export all items in the source bag to the clipboard
// as a JSON string.
//
// action:    the triggered action (unused)
// parameter: unused
// user_data: AppWidgets pointer
static void
on_bag_export(GSimpleAction *action, GVariant *parameter, gpointer user_data)
{
  (void)action; (void)parameter;
  AppWidgets *widgets = user_data;
...
// Async callback: parse JSON from clipboard text and import items into
// the source bag, placing each at its original position or first free spot.
//
// source:    the GdkClipboard
// result:    the async result
// user_data: AppWidgets pointer
static void
on_import_ready(GObject *source, GAsyncResult *result, gpointer user_data)
{
  AppWidgets *widgets = user_data;
...
// GAction callback: import items from clipboard JSON into the source bag.
//
// action:    the triggered action (unused)
// parameter: unused
// user_data: AppWidgets pointer
static void
on_bag_import(GSimpleAction *action, GVariant *parameter, gpointer user_data)
{
  (void)action; (void)parameter;
  AppWidgets *widgets = user_data;
...
// Show the bag context menu popover attached to the given parent widget.
// Builds the menu model with move/copy submenus and direct actions.
//
// widgets:       application state
// parent_widget: the button widget to attach the popover to
// source:        which container type the menu is for
// sack_idx:      index of the sack within the container
void
...
// Register all bag menu GActions with the application.  Each action is
// connected to its callback and added to the app's action map.
//
// app:     the GtkApplication
// widgets: AppWidgets pointer passed as user_data to callbacks
void
