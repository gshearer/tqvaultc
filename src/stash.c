...
// Read a little-endian float32 from data at offset.
// data: source buffer
// offset: byte position to read from
// Returns: the float value
static float
read_f32(const uint8_t *data, size_t offset)
{
  float val;
...
// data: source buffer
// offset: byte position to read from
// file_size: total buffer length for bounds checking
// next_offset: if non-NULL, set to position after the string
// Returns: newly allocated string (caller must free), or NULL
static char *
read_string(const uint8_t *data, size_t offset, size_t file_size,
            size_t *next_offset)
{
...
// data: source buffer
// offset: pointer to current position, advanced past the key
// file_size: total buffer length
// Returns: newly allocated key string, or NULL
static char *
read_key(const uint8_t *data, size_t *offset, size_t file_size)
{
  return(read_string(data, *offset, file_size, offset));
}

// Read a key and validate it matches expected.
// data: source buffer
// offset: pointer to current position, advanced past the key
// file_size: total buffer length
// expected: the key name to match
// Returns: true if key matches expected
static bool
expect_key(const uint8_t *data, size_t *offset, size_t file_size,
           const char *expected)
{
...
// data: source buffer
// offset: pointer to current position, advanced by 4
// file_size: total buffer length
// Returns: the uint32 value, or 0 on bounds error
static uint32_t
read_val_u32(const uint8_t *data, size_t *offset, size_t file_size)
{
  if(*offset + 4 > file_size)
    return(0);
...
// offset: pointer to current position, advanced by 4
// file_size: total buffer length
// Returns: the float value, or 0.0f on bounds error
static float
read_val_f32(const uint8_t *data, size_t *offset, size_t file_size)
{
  if(*offset + 4 > file_size)
    return(0.0f);
...
// data: source buffer
// offset: pointer to current position, advanced past the string
// file_size: total buffer length
// Returns: newly allocated string, or NULL
static char *
read_val_str(const uint8_t *data, size_t *offset, size_t file_size)
{
  return(read_string(data, *offset, file_size, offset));
}
...
// data: source buffer
// offset: byte position to peek at (not advanced)
// file_size: total buffer length
// expected: the key name to match
// Returns: true if the next key matches expected
static bool
peek_key(const uint8_t *data, size_t offset, size_t file_size,
         const char *expected)
{
  if(offset + 4 > file_size)
    return(false);
...
// Append a float32 to the buffer.
// b: target buffer
// val: value to write
static void
bb_write_f32(ByteBuf *b, float val)
{
  bb_write(b, &val, 4);
}
...
// b: target buffer
// s: string to write (NULL treated as empty)
static void
...
// val: float value
static void
bb_write_key_f32(ByteBuf *b, const char *key, float val)
{
  bb_write_str(b, key);
  bb_write_f32(b, val);
}
...
// b: target buffer
// key: key string
// val: string value (NULL treated as empty)
static void
...
// len: number of bytes
static void
bb_write_raw_bytes(ByteBuf *b, const char *data, int len)
{
  bb_write_u32(b, (uint32_t)len);
  if(len > 0)
    bb_write(b, data, (size_t)len);
}
...
static uint32_t
compute_crc32(const uint8_t *data, size_t len)
{
  uint32_t crc = 0;
...
// Read the stash file header fields (begin_block, stashVersion, fName,
// sackWidth, sackHeight, numItems) and populate the TQStash struct.
// data: raw file buffer
// off: pointer to current offset, advanced past all header fields
// sz: total buffer size
// stash: output struct to populate
// num_items: output for number of items read from header
// Returns: true on success, false on parse failure
static bool
stash_read_header(const uint8_t *data, size_t *off, size_t sz,
                  TQStash *stash, int *num_items)
{
  // begin_block
  if(!expect_key(data, off, sz, "begin_block"))
    return(false);
  stash->begin_block_val = read_val_u32(data, off, sz);

  // stashVersion
  if(!expect_key(data, off, sz, "stashVersion"))
    return(false);
  stash->stash_version = (int)read_val_u32(data, off, sz);

  // fName -- raw bytes, not standard CString for the value
  if(!expect_key(data, off, sz, "fName"))
    return(false);
  ...
  {
    stash->stash_name = malloc(name_len + 1);
    if(!stash->stash_name)
      return(false);
    memcpy(stash->stash_name, data + *off, name_len);
    stash->stash_name[name_len] = '\0';
    stash->stash_name_len = (int)name_len;
    *off += name_len;
  }

  // sackWidth
  if(!expect_key(data, off, sz, "sackWidth"))
    return(false);
  stash->sack_width = (int)read_val_u32(data, off, sz);

  // sackHeight
  if(!expect_key(data, off, sz, "sackHeight"))
    return(false);
  stash->sack_height = (int)read_val_u32(data, off, sz);

  // numItems
  if(!expect_key(data, off, sz, "numItems"))
    return(false);
  *num_items = (int)read_val_u32(data, off, sz);
...
// sz: total buffer size
// sack: sack to append the parsed item to
// Returns: true on success, false on parse failure
static bool
...
// filepath: path to the .dxb file
// Returns: newly allocated TQStash, or NULL on failure
TQStash *
stash_load(const char *filepath)
{
  ...
  {
    if(tqvc_debug)
      printf("stash_load: cannot open %s\n", filepath);
    return(NULL);
  }
  ...
  if(!stash_read_header(data, &off, sz, stash, &num_items))
  {
    stash_free(stash);
    free(data);
    return(NULL);
  ...
  if(tqvc_debug)
    printf("stash_load: %s — version=%d, name=%s, %dx%d, %d items\n",
           filepath, stash->stash_version,
           stash->stash_name ? stash->stash_name : "(null)",
           stash->sack_width, stash->sack_height, num_items);
...
// Save a stash to its filepath as a .dxb binary file with CRC32 header.
// Also writes a .dxg backup with the fName extension changed.
// stash: the stash to save
// Returns: 0 on success, -1 on failure
int
stash_save(TQStash *stash)
{
  if(!stash || !stash->filepath)
    return(-1);
...
// type: which stash (transfer, player, relic vault)
// char_folder_name: character folder name, only used for STASH_PLAYER
// Returns: newly allocated path string (caller must free), or NULL on error
char *
