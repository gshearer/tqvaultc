...
static void load_all_difficulties(ChecklistDialogState *st) {
...
static const char *extra_token_for_diff(const ChecklistExtraDef *e, QuestDifficulty d) {
...
static void update_checkboxes(ChecklistDialogState *st) {
...
static GtkWidget *build_category_tab(ChecklistDialogState *st, ChecklistCategory cat) {
    int ecount;
    const ChecklistExtraDef *extras = checklist_get_extras(&ecount);
    ...
    for (int i = 0; i < ecount; i++) {
        if (extras[i].category != cat) continue;
        ...
        if (tok) gtk_widget_set_tooltip_text(cb, tok);
