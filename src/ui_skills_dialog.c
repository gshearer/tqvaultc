...
static int compute_avail(SkillsDialogState *st) {
    int spent = 0;
...
static bool has_changes(SkillsDialogState *st) {
...
/* Find which mastery_defs[] index a skill path belongs to. Returns -1 if none. */
static int find_mastery_for_skill(const char *skill_path) {
...
static bool is_mastery_record(const char *path) {
...
/* Try to resolve a display name tag from a DBR, returning true on success */
static bool resolve_display_tag(AppWidgets *widgets, TQArzRecordData *dbr,
...
static void resolve_skill_name(AppWidgets *widgets, const char *skill_path,
                                char *out, size_t out_size) {
    out[0] = '\0';
...
static bool get_tier_from_ref(const char *ref_field, TQArzRecordData *dbr,
...
static void get_skill_dbr_info(const char *skill_path, int *skill_tier, int *max_level) {
    *skill_tier = 0;
    *max_level = 1;
 ...
 * Returns number of entries. Entries are in tree order (skillName1, skillName2, ...). */
static int load_skill_tree(const char *tree_dbr_path, TreeEntry *entries, int max_entries) {
...
static bool skill_is_accessible(MasteryPane *mp, int skill_idx) {
...
static void zero_skill_and_dependents(SkillsDialogState *st, MasteryPane *mp, int skill_idx) {
    SkillEntry *se = &mp->skills[skill_idx];
    if (se->cur_level > 0) {
        se->cur_level = 0;
...
static void on_skill_plus(GtkButton *btn, gpointer user_data) {
    ...
    if (sidx < 0 || sidx >= mp->num_skills) return;
    SkillEntry *se = &mp->skills[sidx];
    if (se->chr_skill_idx < 0) return; /* not in save */
    if (se->cur_level >= se->max_level) return;
    if (compute_avail(st) <= 0) return;
...
static void on_skill_minus(GtkButton *btn, gpointer user_data) {
    SkillsDialogState *st = user_data;
    int pane = GPOINTER_TO_INT(g_object_get_data(G_OBJECT(btn), "pane"));
    int sidx = GPOINTER_TO_INT(g_object_get_data(G_OBJECT(btn), "sidx"));
    MasteryPane *mp = &st->panes[pane];
...
static void on_mastery_plus(GtkButton *btn, gpointer user_data) {
    SkillsDialogState *st = user_data;
    int pane = GPOINTER_TO_INT(g_object_get_data(G_OBJECT(btn), "pane"));
    MasteryPane *mp = &st->panes[pane];

    if (mp->mastery_level >= MAX_MASTERY_LEVEL) return;
    if (compute_avail(st) <= 0) return;
...
static void on_mastery_minus(GtkButton *btn, gpointer user_data) {
    SkillsDialogState *st = user_data;
    int pane = GPOINTER_TO_INT(g_object_get_data(G_OBJECT(btn), "pane"));
    MasteryPane *mp = &st->panes[pane];

    if (mp->mastery_level <= 0) return;
...
static void on_mastery_changed(GtkDropDown *dd, GParamSpec *pspec, gpointer user_data) {
    (void)pspec;
    SkillsDialogState *st = user_data;
    ...
    if (new_def < 0 || new_def == mp->mastery_def_idx) return;

    /* Check: don't allow selecting the same mastery as the other pane */
    int other = 1 - pane;
    if (st->panes[other].mastery_def_idx == new_def) return;
...
static void skills_state_free(gpointer data) {
...
static GtkWidget *make_skill_row(SkillsDialogState *st, int pane, int sidx) {
...
static void populate_mastery_pane(SkillsDialogState *st, int pane_idx) {
    ...
    if (mp->mastery_def_idx < 0) return;
    ...
    for (int i = 0; i < chr->num_skills; i++) {
        ...
        if (skill_mastery != mp->mastery_def_idx) continue;
    ...
    for (int t = 0; t < num_tree && mp->num_skills < MAX_SKILLS_PER_MASTERY; t++) {
        ...
        for (int i = 0; i < chr->num_skills; i++) {
            if (!chr->skills[i].skill_name) continue;
            char norm[256];
    ...
    for (int i = 0; i < mp->num_skills; i++) {
        if (mp->skills[i].parent_idx >= -99) continue;
    ...
    for (int i = 0; i < num_roots - 1; i++) {
        for (int j = i + 1; j < num_roots; j++) {
            ...
            if (b->skill_tier > a->skill_tier)
                ...
                swap = true;
            if (swap) { int tmp = roots[i]; roots[i] = roots[j]; roots[j] = tmp; }
        }
    }
    ...
    for (int i = 0; i < mp->num_skills; i++) {
        if (!used[i]) display_order[num_display++] = i;
    }
...
static GtkWidget *build_mastery_pane(SkillsDialogState *st, int pane_idx) {
    MasteryPane *mp = &st->panes[pane_idx];
    ...
    for (int m = 0; m < NUM_MASTERIES; m++) {
        ...
        if (st->panes[other_pane].mastery_def_idx == m) continue;
        gtk_string_list_append(model, mastery_defs[m].name);
        if (m == mp->mastery_def_idx) sel_idx = model_idx;
        model_idx++;
    }
...
void show_skills_dialog(AppWidgets *widgets) {
    ...
    for (int i = 0; i < chr->num_skills; i++) {
        if (!is_mastery_record(chr->skills[i].skill_name)) continue;
